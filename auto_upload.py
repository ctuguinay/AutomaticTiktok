...
def uploadVideo(session_id, video, title, tags=[], schedule_time=0, verbose=True):
	...
	for tag in tags:
		...
		try:
			...
			verified_tag = tag
		text += " #"+verified_tag
		text_extra.append({"start":len(text)-len(verified_tag)-1,"end":len(text),"user_id":user_id,"type":1,"hashtag_name":verified_tag})
...
if __name__ == "__main__":
    ...
    title = "Silly Cats"
    video_file_path = "data/videos/03_18_23_sillycats_final.mp4"
    tags = ["Funny", "Joke", "fyp"]
    uploadVideo(session_id, video_file_path, title, tags=tags, verbose=True)